...
RSpec.describe TweetFacade do 
  it 'returns tweets by topic' do 
    ...
    stub_request(:get, "https://api.twitter.com/2/tweets/search/recent?max_results=10&query=nasa%20lang:en&tweet.fields=created_at,lang").
    with(
      headers: {
    'Accept'=>'*/*',
    ...
    results_array.each do |result|
      expect(result[:text].downcase).to include("nasa").or include("space")
    end
